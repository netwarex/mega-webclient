/**
 * Multi-backend Decrypter Worker
...
async function initBackend(backendType) {
    ...
    try {
        ...
        console.log('Decrypter worker initialized with backend:', backendType);
    } catch (e) {
...
/**
 * Legacy ASM.js decryption (for compatibility)
 */
...
/**
 * Modern backend decryption
 */
async function modernDecrypt(data) {
    if (!backend) {
        throw new Error('Backend not initialized');
    }

    const result = await backend.decrypt(key, nonce, ctr, data);
    ...
    try {
        if (typeof e.data === 'string') {
            ...
            if (currentBackendType === CryptoBackends.BACKENDS.ASMJS) {
                result = legacyDecrypt(data);
            } else {
                result = await modernDecrypt(data);
            }
        ...
        console.error('Decrypter worker error:', error);
